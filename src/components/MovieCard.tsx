...
function MovieCard(movie: MovieProps) {
    return (
        <div className="w-[calc(25%-0.75rem)] flex flex-col bg-floral-white rounded-xl overflow-clip">
            <div className={`w-full aspect-square bg-cover bg-center bg-amber-200`}
                style={{ backgroundImage: `url(${movie.cover})` }}>
            </div>
            <div className='flex flex-1 flex-col justify-between p-2.5 font-bold'>
                <h2 className='text-lg text-ellipsis line-clamp-2 leading-5'>{movie.name}</h2>
                <p className='text-base text-grey '>{movie.year}</p>
